...
describe('BloomFilter', function() {
  ...
  // test data from: https://github.com/aipgproject/aipgcoin/blob/master/src/test/bloom_tests.cpp

  it('serialize filter with public keys added', function() {

    var privateKey = aipgcore.PrivateKey.fromWIF('5Kg1gnAjaLfKiwhhPpGS3QfRg2m6awQvaj98JCZBZQ5SuS2F15C');
    var publicKey = privateKey.toPublicKey();

    var filter = BloomFilter.create(2, 0.001, 0, BloomFilter.BLOOM_UPDATE_ALL);
    filter.insert(publicKey.toBuffer());
    filter.insert(aipgcore.crypto.Hash.sha256ripemd160(publicKey.toBuffer()));
